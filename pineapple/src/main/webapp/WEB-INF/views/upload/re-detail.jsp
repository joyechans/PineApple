...
" />
		        	...
		        	window.addEventListener('load',function(event){//js의 main 함수 역할
		        		...
		        		btnCancel.addEventListener('click', function(event) {
		        			location.href="/pineapple/upload/re-list";//주소창에 list을 입력하고 엔터
		        		...
		        		btnDelete.addEventListener('click', function(event){
		        			...
		        			location.href="/pineapple/upload/delete/${ review.reviewNo }";
		        			}
		        		});
		        		...
		        		btnUpdate.addEventListener('click', function(event){
		        			//<a 를 통한 요청이므로 주소 뒤에 ?key=value 형식을 써서 데이터 전송
		        			location.href="/pineapple/upload/re-update/${ review.reviewNo }";
		        		});
	...
	$(function() {
		$('#writecomment').on('click', function(event) {
			...
			// alert(formData)
 			...
 			$.ajax({
				url: "/pineapple/upload/write-comment",
				method: "POST",
				data: formData,
		...
		$('#comment-list').on('click', '.deletecomment', function(event) {
			...
			$.ajax({
				url: "/pineapple/upload/delete-comment",
				method: "GET",
		...
		$('#comment-list').on('click', '.updatecomment', function(event) {
			...
			//ajax 방식으로 데이터 수정
			$.ajax({
				"url": "/pineapple/upload/update-comment",
				"method": "POST",
				"data": inputData,
				"success": function(data, status, xhr) {
					alert('댓글을 수정했습니다.');
		...
		$('#write-recomment').on('click', function(event) {
			...
			$.ajax({
				url: "/pineapple/upload/write-recomment",
				method: "POST",
				data: recommentData,
				success: function(data, status, xhr) {
					alert('success');					
